...
public class groupByDays extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ArrayList<RoomMessage> Days, RoomMessages;
    ...
    // 适配器
    private MyExpandableListViewAdapter_Days mAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Days = new ArrayList<RoomMessage>();
        ...
        mAdapter = new MyExpandableListViewAdapter_Days(this, mGroupNameList, mItemNameList);
        ...
        mExpandableListView.setOnItemLongClickListener(new ExpandableListView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (childPosition != -1) {
                    ...
                    normalDialog.setIcon(R.drawable.date);
                    ...
                    normalDialog.setNegativeButton("预约", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (IsMeeting.equals("占用")) {
                                ZLoadingDialog dialog2 = new ZLoadingDialog(groupByDays.this);
                                ...
                                Intent intent = new Intent(groupByDays.this, addPerson_handler.class);
    ...
    private void initData() {
        ...
        List<RoomMessage> data1 = selectDays();
        mItemNameList = new ArrayList<List<RoomMessage>>();
        List<RoomMessage> itemList = new ArrayList<RoomMessage>();
        for (int i = 0; i < data1.size(); i++) {
            mGroupNameList.add(data1.get(i).getDays());
            Log.d("fff", "selectDays   查出来的Days   " + i + data1.get(i).getDays());
            Log.d("fff", String.valueOf(data1.size()));
        }
        for (int i = 0; i < data1.size(); i++) {
            Log.d("ddd", "第几次   " + i);
            String Days2 = data1.get(i).getDays();
            Log.d("ddd", "第几次   " + Days2);
            itemList = selectAllRoomByDays(Days2);
            Log.d("ddd", "执行  selectAllRoomByDays  " + Days2);
...
//        deleteDays();
    ...
    public ArrayList<RoomMessage> selectDays() {
        Log.d("fff", "jinru");
        SQLiteDatabase db = helper4.getReadableDatabase();
        Cursor cursor = db.rawQuery("select distinct * from Days", null);
        String Days1 = null;
        while (cursor.moveToNext()) {
            RoomMessage data1 = new RoomMessage();
            Days1 = cursor.getString(cursor.getColumnIndex("Days"));
            data1.setDays(Days1);
            Days.add(data1);
        }
        db.close();
    ...
    public ArrayList<RoomMessage> selectAllRoomByDays(String Days1) {
        SQLiteDatabase db = helper4.getReadableDatabase();
        ArrayList<RoomMessage> RoomMessages2 = new ArrayList<RoomMessage>();
        Cursor cursor = db.rawQuery("select * from allroom where Days =?", new String[]{Days1});
        ...
        while (cursor.moveToNext()) {
            ...
            Log.d("ddd", "selectAllRoomBySizes 里的" + Size);

...
public class groupByTime extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ArrayList<RoomMessage> Times, RoomMessages;
    ...
    // 适配器
    private MyExpandableListViewAdapter_Time mAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Times = new ArrayList<RoomMessage>();
        ...
        mAdapter = new MyExpandableListViewAdapter_Time(this, mGroupNameList, mItemNameList);
        ...
        mExpandableListView.setOnItemLongClickListener(new ExpandableListView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (childPosition != -1) {
                    ...
                    normalDialog.setIcon(R.drawable.time);
                    ...
                    normalDialog.setNegativeButton("预约", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (IsMeeting.equals("占用")) {
                                ZLoadingDialog dialog2 = new ZLoadingDialog(groupByTime.this);
                                ...
                                Intent intent = new Intent(groupByTime.this, addPerson_handler.class);
    ...
    private void initData() {
        ...
        List<RoomMessage> data1 = selectTimes();
        mItemNameList = new ArrayList<List<RoomMessage>>();
        List<RoomMessage> itemList = new ArrayList<RoomMessage>();
        for (int i = 0; i < data1.size(); i++) {
            mGroupNameList.add(data1.get(i).getTime());
            Log.d("ggg", "selectDays   查出来的Days   " + i + data1.get(i).getDays());
            Log.d("ggg", String.valueOf(data1.size()));
        }
        for (int i = 0; i < data1.size(); i++) {
            Log.d("ggg", "第几次   " + i);
            String Days2 = data1.get(i).getTime();
            Log.d("ggg", "第几次   " + Days2);
            itemList = selectAllRoomByTimes(Days2);
            Log.d("ggg", "执行  selectAllRoomByDays  " + Days2);
...
//        deleteTimes();
    ...
    public ArrayList<RoomMessage> selectTimes() {
        Log.d("ggg", "jinru");
        SQLiteDatabase db = helper4.getReadableDatabase();
        Cursor cursor = db.rawQuery("select distinct * from Time", null);
        String Times1 = null;
        while (cursor.moveToNext()) {
            RoomMessage data1 = new RoomMessage();
            Times1 = cursor.getString(cursor.getColumnIndex("Time"));
            data1.setTime(Times1);
            Times.add(data1);
        }
        db.close();
        return Times;
    ...
    public ArrayList<RoomMessage> selectAllRoomByTimes(String Times1) {
        SQLiteDatabase db = helper4.getReadableDatabase();
        ArrayList<RoomMessage> RoomMessages2 = new ArrayList<RoomMessage>();
        Cursor cursor = db.rawQuery("select * from allroom where Time =?", new String[]{Times1});
        ...
        while (cursor.moveToNext()) {
            ...
            Log.d("ggg", "selectAllRoomByTimes 里的" + days);

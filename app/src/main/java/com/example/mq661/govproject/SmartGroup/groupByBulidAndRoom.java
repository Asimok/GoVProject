...
public class groupByBulidAndRoom extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ArrayList<RoomMessage> BuildAndRoom, RoomMessages;
    private ExpandableListView mExpandableListView = null;
    // 列表数据
    private List<String> mGroupNameList = null;
    private List<List<RoomMessage>> mItemNameList = null;
    // 适配器
    private MyExpandableListViewAdapter_BuildAndRoom mAdapter = null;
    private roomSortDBHelper helper4;
    private ZLoadingDialog dialog;
    private SwipeRefreshLayout mSwipeRefreshLayout = null;
    //private  String BuildingNumber,RoomNumber,Days,Time;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BuildAndRoom = new ArrayList<RoomMessage>();
        ...
        mAdapter = new MyExpandableListViewAdapter_BuildAndRoom(this, mGroupNameList, mItemNameList);
        ...
        mExpandableListView.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition,
                ...
                String a = mItemNameList.get(groupPosition).get(childPosition).getBuildingNumber();
//                ToastUtil.showToast(groupByBulidAndRoom.this,a);
                return false;
            }
        });
                    ...
                    normalDialog.setNegativeButton("预约", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            if (IsMeeting.equals("占用")) {
                                ZLoadingDialog dialog2 = new ZLoadingDialog(groupByBulidAndRoom.this);
    ...
    private void initData() {
        ...
        List<RoomMessage> data1 = selectBuildAndRoom();
        mItemNameList = new ArrayList<List<RoomMessage>>();
        List<RoomMessage> itemList = new ArrayList<RoomMessage>();
        for (int i = 0; i < data1.size(); i++) {
            mGroupNameList.add(data1.get(i).getBuildingNumber() + "  " + data1.get(i).getRoomNumber());
        }
        for (int i = 0; i < data1.size(); i++) {
            Log.d("ddd", "第几次   " + i);
            String BuildingNumber2 = data1.get(i).getBuildingNumber();
            String RoomNumber2 = data1.get(i).getRoomNumber();
            itemList = selectAllRoomByBuildAndRoom(BuildingNumber2, RoomNumber2);
...
//        deleteBuildAndRoom();
    ...
    public ArrayList<RoomMessage> selectBuildAndRoom() {
        Log.d("fff", "jinru");
        SQLiteDatabase db = helper4.getReadableDatabase();
        Cursor cursor = db.rawQuery("select distinct * from BuildAndRoomNumber", null);
        String BuildNumber1 = null;
        String RoomNumber1 = null;
        while (cursor.moveToNext()) {
            RoomMessage data1 = new RoomMessage();
            BuildNumber1 = cursor.getString(cursor.getColumnIndex("BuildNumber"));
            RoomNumber1 = cursor.getString(cursor.getColumnIndex("RoomNumber"));
            data1.setBuildingNumber(BuildNumber1);
            data1.setRoomNumber(RoomNumber1);
            BuildAndRoom.add(data1);
        }
        db.close();
        return BuildAndRoom;
    }

    public ArrayList<RoomMessage> selectAllRoomByBuildAndRoom(String BuildNumber1, String RoomNumber1) {
        SQLiteDatabase db = helper4.getReadableDatabase();
        ArrayList<RoomMessage> RoomMessages2 = new ArrayList<RoomMessage>();
        Cursor cursor = db.rawQuery("select * from allroom where BuildNumber=? and RoomNumber=?", new String[]{BuildNumber1, RoomNumber1});
        ...
        while (cursor.moveToNext()) {
            ...
            Log.d("ddd", "selectAllRoomByBuildAndRoom 里的" + Size);
